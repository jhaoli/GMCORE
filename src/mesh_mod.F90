...
!     end do
!     mesh%cell_area(1) = radius**2 * mesh%half_cos_lat(1) * mesh%dlon * mesh%dlat * 0.25
!     mesh%cell_area(mesh%num_full_lat) = radius**2 * mesh%half_cos_lat(mesh%num_half_lat) * mesh%dlon * mesh%dlat * 0.25 
  
!     do j = 1, mesh%num_half_lat
!       mesh%cell_area(j) = radius**2 * mesh%half_cos_lat(j) * mesh%dlon * mesh%dlat
!     end do 
...
!  integrate compute the cell area
    ...
    mesh%cell_area(1) = radius**2 * mesh%dlon * ( mesh%half_sin_lat(1) + 1)
    mesh%cell_area(mesh%num_full_lat) = radius**2 * mesh%dlon * (1 - mesh%half_sin_lat(mesh%num_full_lat-1))
