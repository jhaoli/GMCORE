...
contains
  ...
  subroutine history_init()
    ...
    call io_add_var('tm',   long_name='total mass',                units='m2    ', dim_names=['time'])
    call io_add_var('tes',  long_name='total potential enstrophy', units='m-1 s-2',dim_names=['time'])
    call io_add_var('tav',  long_name='total absolute vorticity',  units='m2 s-3', dim_names=['time'])
...
!     call io_add_var('iap_v',        'debug', long_name='IAP v',               units='', dim_names=['lon ', 'ilat', 'time'])
    ...
    call io_add_var('normal_lat_flux', 'debug', long_name='normal lat flux',      units='', dim_names=['lon ', 'ilat', 'time'])
