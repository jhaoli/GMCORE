...
!     call io_add_var('iap_v',        'debug', long_name='IAP v',               units='', dim_names=['lon ', 'ilat', 'time'])
    ...
    call io_add_var('normal_u_flux', 'debug', long_name='normal u flux',      units='', dim_names=['ilon', 'lat ', 'time'])
    call io_add_var('normal_v_flux', 'debug', long_name='normal v flux',      units='', dim_names=['lon ', 'ilat', 'time'])
    ...
    call io_add_var('tangen_u_flux','debug', long_name='tangential_flux on u',units='', dim_names=['ilon', 'lat ', 'time'])
    call io_add_var('tangen_v_flux','debug', long_name='tangential_flux on v',units='', dim_names=['lon ', 'ilat', 'time'])
  ...
  subroutine history_write_tendency(state, tend, tag)
    ...
    call io_output('u_pgf',         tend%u_pgf(1:mesh%num_half_lon,1:mesh%num_full_lat),        'debug')
    call io_output('v_pgf',         tend%v_pgf(1:mesh%num_full_lon,1:mesh%num_half_lat),        'debug')
    call io_output('mass_div_lon',  tend%mass_div_lon(1:mesh%num_full_lon,1:mesh%num_full_lat), 'debug')
    call io_output('mass_div_lat',  tend%mass_div_lat(1:mesh%num_full_lon,1:mesh%num_full_lat), 'debug')
    ...
    call io_output('normal_u_flux', tend%diag%normal_u_flux(1:mesh%num_half_lon,1:mesh%num_full_lat), 'debug')
    call io_output('normal_v_flux', tend%diag%normal_v_flux(1:mesh%num_full_lon,1:mesh%num_half_lat), 'debug')
    ...
    call io_output('tangen_u_flux', tend%diag%tangential_u_flux(1:mesh%num_half_lon,1:mesh%num_full_lat), 'debug')
    call io_output('tangen_v_flux', tend%diag%tangential_v_flux(1:mesh%num_full_lon,1:mesh%num_half_lat), 'debug')
