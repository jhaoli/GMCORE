...
!     call io_add_var('fu',           'debug', long_name='fu',                  units='', dim_names=['lon ', 'ilat', 'time'])
    call io_add_var('dKdlon',  'debug', long_name='zonal kinetic gradient',      units='', dim_names=['ilon', 'lat ', 'time'])
    call io_add_var('dKdlat', 'debug', long_name='meridional kinetic gradient',  units='', dim_names=['lon ', 'ilat', 'time'])
    call io_add_var('dPdlon', 'debug', long_name='zonal potential gradient',     units='', dim_names=['ilon', 'lat ', 'time'])
    call io_add_var('dPdlat', 'debug', long_name='meridional potential gradient',units='', dim_names=['lon ', 'ilat', 'time'])
...
!     call io_add_var('tangent_gradient_pv_lat', 'debug', long_name='tangent_gradient_pv_lat',units='', dim_names=['lon ', 'ilat', 'time'])
  ...
  subroutine history_write_tendency(state, tend, tag)
    ...
    call io_output('dKdlon',         tend%dKdlon(1:mesh%num_half_lon,1:mesh%num_full_lat),        'debug')
    call io_output('dKdlat',         tend%dKdlat(1:mesh%num_full_lon,1:mesh%num_half_lat),        'debug')
    call io_output('dPdlon',         tend%dPdlon(1:mesh%num_half_lon,1:mesh%num_full_lat),        'debug')
    call io_output('dPdlat',         tend%dPdlat(1:mesh%num_full_lon,1:mesh%num_half_lat),        'debug')
