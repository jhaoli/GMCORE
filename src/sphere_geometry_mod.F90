...
contains
  ...
  real function calc_area_with_last_small_arc(x, y, z) result(res)
    ...
    if (abs(lat1)< 1.0E-12) then
      ...
      area3 = area1 - area2
      if (abs(area3) < 1.0e-12) call log_error('Lune area is negative!', __FILE__, __LINE__)
      ...
      if (abs(res) < 1.0E-12) call log_error('Failed to calculate area with small arc!', __FILE__, __LINE__)
    end if 
