...
contains
  ...
  subroutine allocate_tend_data(tend)
    ...
    if (.not. allocated(tend%diag%normal_u_flux))  call parallel_allocate(tend%diag%normal_u_flux, half_lon=.true.)
  ...
  subroutine deallocate_tend_data(tend)
    ...
    if (allocated(tend%diag%normal_u_flux))  deallocate(tend%diag%normal_u_flux)
